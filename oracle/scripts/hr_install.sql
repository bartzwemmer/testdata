...
-- Fetch default tablespace name
COLUMN default_tbs NEW_VALUE var_default_tbs NOPRINT
SELECT property_value AS default_tbs FROM database_properties WHERE property_name = 'DEFAULT_PERMANENT_TABLESPACE';

-- Logic to choose between provided &tbs and default
COLUMN final_tbs NEW_VALUE var_final_tbs NOPRINT
SELECT CASE 
         WHEN '&tbs' IS NOT NULL AND '&tbs' != '""' AND '&tbs' != ' ' THEN UPPER('&tbs')
         ELSE '&var_default_tbs'
       END AS final_tbs 
FROM dual;
...
BEGIN
   SELECT COUNT(1) INTO v_tbs_exists
     FROM DBA_TABLESPACES
       WHERE TABLESPACE_NAME = '&var_final_tbs';
   IF v_tbs_exists = 0 THEN
      RAISE_APPLICATION_ERROR(-20998, 'Error: the tablespace ''&var_final_tbs'' does not exist!');
   END IF;
END;
/
...
CREATE USER hr IDENTIFIED BY "&pass"
               DEFAULT TABLESPACE &var_final_tbs
               QUOTA UNLIMITED ON &var_final_tbs;
